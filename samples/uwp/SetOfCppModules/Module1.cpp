...
void SetOfCppModules::Module1::Create(Microsoft::Azure::IoT::Gateway::MessageBus ^bus, Windows::Foundation::Collections::IMapView<Platform::String^, Platform::String^>^ configuration)
{
	...
	create_async([bus]() {
		...
		for (int i = 0; i < 3; i++)
		{
			...
			Platform::String^ content = "SetOfCppModules::Module1 says hello!!!";
			auto properties = ref new Platform::Collections::Map<Platform::String^, Platform::String^>();

			swprintf(count, 3, L"%d", i);
			properties->Insert("SetOfCppModules::Module1", ref new Platform::String(count));
...
void SetOfCppModules::Module1::Receive(Microsoft::Azure::IoT::Gateway::Message ^received_message)
{
	...
	std::wstring output = L"SetOfCppModules.Module1.Receive: Content=";
	output += content.c_str();
	output += L"\r\n";
	output += L"SetOfCppModules.Module1.Receive: Properties={";
